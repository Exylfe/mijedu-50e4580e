      ...
      <AnimatePresence>
        {showMenu && (
          <>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: -10 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }} className="fixed right-4 z-50 min-w-56 rounded-2xl p-2 shadow-xl" style={{ top: 'calc(env(safe-area-inset-top, 0px) + 4rem)' }} style={{ background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(15px)', WebkitBackdropFilter: 'blur(15px)', border: '0.5px solid hsl(var(--border) / 0.5)' }}>
              {profile && user && (
